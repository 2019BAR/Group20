...
```{r}
plot(CK$Date, CK$StockPrice, type='l') 
...
legend("topright",legend=c("Cocacola","P&G"),col=c('blue','green'),lwd=2)
#P&G
```
...
abline(v = as.Date("2000-03-01"), lty=3, col='black')
abline(v = as.Date("1997-09-01"), lty=3, col='black')
abline(v = as.Date("1997-11-01"), lty=3, col='black')
legend("topright",legend=c("Cocacola","P&G","IBM","GE","BE"),col=c('red','blue','green','purple','orange'),lwd=5)
#GE
```
...
```{r}
sort(tapply(IBM$Stock, format(IBM$Date,"%B"), mean))
#February
```
...
```{r}
sapply(list(CK=CK,GE=GE), function(df) {tapply(df$Stock,format(df$Date,"%m"), mean) - mean(df$Stock)})  
# April
```
...
```{r}
sapply(list(BE=BE,CK=CK,GE=GE,IBM=IBM,PG=PG), function(df) {tapply(df$Stock,format(df$Date,"%m"), mean) - mean(df$Stock)})  
#December
```
