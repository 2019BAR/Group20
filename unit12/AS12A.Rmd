...
&emsp; ■ blue:機率f(x)=b0+b1x1+b2x2=0的logistic regression <br>
&emsp; ■ 以blue線為界:以上的cyan線:機率f(x)=b0+b1x1+b2x2>0的logistic regression，以下的cyan線:機率f(x)=b0+b1x1+b2x2<0的logistic regression <br>
<span style="font-size:24px"> `r "\U1F5FF"` : </span>
Given any point in the figure above, how can you tell its (predicted) probability approximately?<br>
&emsp; ■ x1、x2帶入預測迴歸方程式所得的f(x)為probability<br>
...
&emsp; ■ he logit is a link function / a transformation of a parameter. It is the logarithm of the odds.The logistic function is the inverse of the logit.<br>
&emsp; ■logit(π)=log(π/1-π) , logistic(x)=e^x/1+e^x<br>
