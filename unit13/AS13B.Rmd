...
```{r}
c(trAUC, tsAUC) #判斷glm模型對這個數據的配適模式程度好不好
```
<br><hr>
...
TR = subset(D, split == TRUE)
TS = subset(D, split == FALSE)
glm1 = glm(PoorCare ~ OfficeVisits + Narcotics+DaysSinceLastERVisit+ERVisits+Pain+TotalVisits+ProviderCount+MedicalClaims+ClaimLines+AcuteDrugGapSmall+InpatientDays, TR, family=binomial)
summary(glm1)
```
...
&emsp;  Ans:有，test0.81從0.812變為0.844 <br>
&emsp; 【F】 為什麼它比較準(或比較不準)呢？<br><br>
&emsp;  Ans:因為加入的變數變多 所以以test的data來說，準確性上升的機率變大<br>
</p class="qiz">
